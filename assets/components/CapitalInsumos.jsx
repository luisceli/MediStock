...
const CapitalInsumos=()=>{
    ...
    const fetchCostoTotal = async () => {
      try {
        const costoTotalResponse = await axios.get(`${baseUrl}insumos/get/costototal`);
    ...
    const filteredData = historicoData.filter((item) => {
        return item.estimacion_Cantidad > 0 && item.estimacion_Costo > 0 && item.nombre_Generico.toLowerCase().includes(searchText.toLowerCase());
      });
